...
| --- | --- | --- | --- |
| `baseerkhan/nr-lesson-final` | Streamlit UI, FastAPI tool endpoint, XLSX persistence, model/API boundary, multiple trust boundaries | Legacy dependency set; existing UI accepts an OpenAI key | Selected. It is the smallest public app that supports a factual, non-trivial Application Model and a useful threat-triage comparison. The Avcel experiment is isolated in a new server-only Node application and does not reuse the legacy browser-key pattern. |
...
The implementation therefore lives in the clean repository `baseerkhan/avcel-jev-threat-model-demo` with a fresh history that does not inherit the subject repository's compromised commit.
