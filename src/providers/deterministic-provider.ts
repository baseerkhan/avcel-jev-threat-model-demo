...
const rules: Record<string, Rule> = {
  ...
  "PF-11": {
    unlikelyWhen: (state) => !state.applicationModel.selectedFlow.data.some((item) => item.toLowerCase().includes("file")),
...
const relevantIds = (state: ThreatEvaluationState): Set<string> =>
  new Set([
    ...state.applicationModel.relevantFacts.map((fact) => fact.id),
    ...state.applicationModel.knownUnknowns.map((fact) => fact.id),
  ]);
