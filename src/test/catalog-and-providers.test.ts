...
test("state selects one flow and preserves source-backed facts and unknowns", () => {
  ...
  assert.equal(state.applicationModel.selectedFlow.id, "streamlit-to-mcp");
  assert.equal(state.applicationModel.commit, applicationModel.commit);
  assert.ok(state.applicationModel.relevantFacts.some((fact) => fact.id === "auth-not-found"));
  assert.ok(state.applicationModel.knownUnknowns.length > 0);
...
test("run record keeps lineage, both machine paths, and human authority separate", async () => {
  ...
  const runRecord = createRunRecord(record);
  assert.equal(runRecord.lineage.commit, applicationModel.commit);
  assert.equal(runRecord.decisions.humanReview, null);
  assert.equal(runRecord.authority.status, "review-required");
  assert.match(JSON.stringify(runRecord), /conventional generative LLM was intentionally not run/);
});
