...
import { factById, applicationModel } from "./application-model.js";
...
export function buildState(flowId: string): ThreatEvaluationState {
  const selectedFlow = applicationModel.flows.find((flow) => flow.id === flowId);
  if (!selectedFlow) {
  ...
  const knownUnknowns = applicationModel.facts.filter((fact) => fact.kind === "unknown");
...
export function buildQuestions(): ThreatQuestionSet {
  return Object.fromEntries(
    threatCatalog.map((threat) => [
      ...
      noul(
        {
          task: `Judge whether ${threat.id} ${threat.name} is materially applicable to \`applicationModel.selectedFlow\`.`,
