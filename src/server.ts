...
const dataDirectory = process.env.AVCEL_DATA_DIR || join(process.cwd(), "data");
const maxRecords = Number(process.env.AVCEL_MAX_RECORDS || "250");
...
async function handle(request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  if (request.method === "GET" && pathname === "/api/context") {
    return json(response, 200, {
      ...
      requestStates: Object.fromEntries(
        applicationModel.flows.map((flow) => [flow.id, buildState(flow.id)]),
      ),
      catalogVersion: threatCatalogVersion,
      methodology: {
  ...
  if (request.method === "POST" && pathname === "/api/evaluations") {
    ...
    if (typeof body.flowId !== "string" || !applicationModel.flows.some((flow) => flow.id === body.flowId)) {
      return json(response, 400, { error: "Choose a documented application flow." });
    }
  ...
  const recordMatch = pathname.match(/^\/api\/evaluations\/([a-f0-9-]{36})\/run-record$/);
  if (request.method === "GET" && recordMatch?.[1]) {
    const record = await store.get(recordMatch[1]);
    if (!record) return json(response, 404, { error: "Evaluation not found." });
    securityHeaders(response);
    response.statusCode = 200;
    response.setHeader("Content-Type", "application/json; charset=utf-8");
    response.setHeader("Content-Disposition", `attachment; filename=avcel-run-record-${record.id}.json`);
    response.end(`${JSON.stringify(createRunRecord(record), null, 2)}\n`);
    return;
  }
...
server.listen(port, host, () => {
  process.stdout.write(`Avcel experiment listening on http://${host}:${port}\n`);
});
