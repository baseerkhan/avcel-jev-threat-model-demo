...
function renderContext(context) {
  $("#short-sha").textContent = context.applicationModel.commit.slice(0, 12);
  const select = $("#flow-select");
  select.replaceChildren();
  for (const flow of context.applicationModel.flows) {
  ...
  const facts = context.applicationModel.facts.filter((fact) =>
  ...
  for (const fact of context.applicationModel.facts) sources.set(fact.provenance.id, fact.provenance);
...
function updateFlowDescription() {
  ...
  const flow = state.context.applicationModel.flows.find((item) => item.id === $("#flow-select").value);
...
function renderEvaluation(record) {
  ...
  $("#download-record").href = apiUrl(`evaluations/${record.id}/run-record`);
